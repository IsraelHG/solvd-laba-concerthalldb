...
public class EventsDAOImpl extends BaseDAOImpl<Events> implements EventsDAO {
    ...
    @Override
    protected Events mapRow(ResultSet resultSet) throws SQLException {
    ...
    @Override
    protected int getId(Events entity) {
        return entity.getEventId();
    }

    @Override
    protected String getInsertValues(Events entity) {
        return entity.getEventId() + "', '" + entity.getName() + "', '" + entity.getStartDate() + "', '"
                + entity.getEndDate();
    }

    @Override
    protected String getUpdateValues(Events entity) {
        return "name = '" + entity.getName() + "', startDate = '" + entity.getStartDate() + "', endDate = '"
                + entity.getEndDate();
    }

    @Override
    public ArrayList<Events> fetchEventsForVenue(int venueId) {
        ...
        try (Connection connection = Database.getConnection(); PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            while (resultSet.next()) {
                ...
                Events event = fetchEventById(eventId);
                if (event != null) {
     ...
     */
    private Events fetchEventById(int eventId) {
        ...
        try (Connection connection = Database.getConnection(); PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            if (resultSet.next()) {
                ...
                return new Events(fetchedEventId, name, startDate, endDate);
            }
        } catch (SQLException e) {
            // Handle any potential database access errors
            e.printStackTrace();
